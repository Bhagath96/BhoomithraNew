                    ...
                    <Accordion defaultExpanded={true} >
                        ...
                        <Grid container spacing={2} key={index} style={renderCard.gridCenter}>
                            <Grid item xs={shortLabelDisplay ? 3 : 4} >
                                <Field name={`${member}.langId`} label="language" component={renderSimpleSelect} onChange={(value) =>
    ...
    const submit = (values) => {
        ...
        if (values.name && values.code && values.sort) {
            if (id) {
                ...
                if (values.labels?.length > 0) {
                    object.labels = values.labels;
                    dispatch(Actions.saveStateData(object));
                } else {
                    SetLabelError('At least one label must be entered');
                }
            ...
            SetFormError('check you are entered data');
        }
    };
            ...
            <Form onSubmit={handleSubmit(submit)} autoComplete='off'>
                <Grid container>
                    <Grid item xs={6} sm={6} md={6} className={classes.item}>
                        <Field name='code' label="Code" type='text' component={renderTextField} />
                    </Grid>
                    <Grid item xs={6} sm={6} md={6} className={classes.item}>
                        <Field name='name' label="Name" type='text' component={renderTextField} />
                    </Grid>
                    <Grid item xs={6} sm={6} md={6} className={classes.item}>
                        <Field name='sort' label="Sort Order" type='number' component={renderTextField} />
                    </Grid>
