...
import { deleteOrganisationalRole, setRoleType, setRole } from '../actions';
...
function OrganisationRoles(props) {
    ...
    const passedColumns = [
        ...
        { columnLabel: 'role_name', columnName: 'name', apiParam: 'names', filterBy: 'name' },
    ...
    const deletePressed = (rowData) => {
        ...
        Swal.fire({
            ...
            if (result.value) {
                dispatch(deleteOrganisationalRole(id || 0, page, size, count));
            }
        });
    };
    ...
    const showActionMenu = hasAccessPermission(RESOURCE_MAPPING.ROLES_ORGANIZATIONAL, ACTION_MAPPING.ROLES_ORGANIZATIONAL.EDIT_IN_ACTION) || hasAccessPermission(RESOURCE_MAPPING.ROLE, ACTION_MAPPING.ROLES_ORGANIZATIONAL.DELETE_IN_ACTION) || false;
    const showAddIcon = hasAccessPermission(RESOURCE_MAPPING.ROLES_ORGANIZATIONAL, ACTION_MAPPING.ROLES_ORGANIZATIONAL.ADD) || false;
    if (showActionMenu) {
        let actions = {
            name: 'Actions',
            ...
            options: {
                ...
                customBodyRender: (value, tableMeta) => {
                    ...
                    let menuActions = [];
                    if (hasAccessPermission(RESOURCE_MAPPING.ROLES_ORGANIZATIONAL, ACTION_MAPPING.ROLES_ORGANIZATIONAL.EDIT_IN_ACTION)) {
                        menuActions.push({ name: I18n.t('edit'), fn: () => editPressed(rowData) });
                    }
                    if (hasAccessPermission(RESOURCE_MAPPING.ROLES_ORGANIZATIONAL, ACTION_MAPPING.ROLES_ORGANIZATIONAL.DELETE_IN_ACTION)) {
