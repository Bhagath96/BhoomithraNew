...
function ListRegularRole(props) {
    ...
    const showActionMenu = hasAccessPermission(RESOURCE_MAPPING.ROLES_REGULAR, ACTION_MAPPING.ROLES_REGULAR.EDIT_IN_ACTION) || hasAccessPermission(RESOURCE_MAPPING.ROLES_REGULAR, ACTION_MAPPING.ROLES_REGULAR.DELETE_IN_ACTION) || true;
    const showAddIcon = hasAccessPermission(RESOURCE_MAPPING.ROLES_REGULAR, ACTION_MAPPING.ROLES_REGULAR.ADD);
    if (showActionMenu) {
        let actions = {
            name: 'Actions',
            ...
            options: {
                ...
                customBodyRender: (value, tableMeta) => {
                    ...
                    let menuActions = [];
                    if (hasAccessPermission(RESOURCE_MAPPING.ROLES_REGULAR, ACTION_MAPPING.ROLES_REGULAR.EDIT_IN_ACTION)) {
                        menuActions.push({ name: I18n.t('edit'), fn: () => editPressed(rowData) });
                    }
                    if (hasAccessPermission(RESOURCE_MAPPING.ROLES_REGULAR, ACTION_MAPPING.ROLES_REGULAR.DELETE_IN_ACTION)) {
