...
function AssignCustomerToServiceWorker(props) {
    ...
    const { listAllServiceProviders: { data: serviceProviders = [] } = {}, wardsByProviderId = {}, swSuperVisorForWard = {}, serviceWorkerFromSwSuperVisor, assignCustomerToServiceWorker: { data: assignCustomerToServiceWorkerData = [] } = {} } = useSelector(state => state[STATE_REDUCER_KEY]);
    ...
    const submit = (values) => {
        if (values.allSelected) {
            ...
            dispatch(Actions.submitcustomerNumbers({ customerToServiceWorkerObj, organizationId: id, serviceProviderId: currentProvider?.id, supervisorId: currentSwSuperVisor?.id, gtId: currentServiceWorkerFromSuperVisor?.id, allSelected: true, wardId: currentWard?.id }));
            ...
            dispatch(Actions.submitcustomerNumbers({ customerToServiceWorkerObj, organizationId: id, serviceProviderId: currentProvider?.id, supervisorId: currentSwSuperVisor?.id, gtId: currentServiceWorkerFromSuperVisor?.id, allSelected: false, wardId: currentWard?.id }));
        }
    };
    ...
    const onMemberClick = (params) => {
        ...
        if (currentWard?.id && currentSwSuperVisor?.id) {
            conditionalReload({ unassignedUsersOnly, member: flag, organizationId: Number(id), serviceProviderId: Number(currentProvider?.id), wardId: currentWard?.id, gtId: currentServiceWorkerFromSuperVisor?.id, apiCallFlag: true, supervisorId: currentSwSuperVisor?.id });
        }
    };
    ...
    useEffect(() => {
        dispatch(Actions.getAllServiceProviderUnderOrg({ organizationId: id }));
    ...
    const readOnlyPermission = hasAccessPermission(RESOURCE_MAPPING.ORGANIZATION_ASSIGN_CUSTOMER_TO_SERVICE_WORKER, ACTION_MAPPING.ORGANIZATION_ASSIGN_CUSTOMER_TO_SERVICE_WORKER.READ_ONLY);
                        ...
                        return (
                            <>
                                ...
                                <PickySelect
                                    ...
                                    dropdownHeight={600}
                                    getFilterValue={val => {
                                        getDropdownFilterList(val, _.get(searchKeys, passedColumns[1].columnName, 'customer.customerName'), [passedColumns[1].columnName], id, currentServiceWorkerFromSuperVisor?.id, currentWard?.id, currentSwSuperVisor?.id, currentProvider?.id);
                                    }}
                                />
                            </>
                        );
                        ...
                        return (
                            <>
                                ...
                                <PickySelect
                                    ...
                                    dropdownHeight={600}
                                    getFilterValue={val => {
                                        getDropdownFilterList(val, _.get(searchKeys, passedColumns[2].columnName, 'customer.phoneNumber'), [passedColumns[2].columnName], id, currentServiceWorkerFromSuperVisor?.id, currentWard?.id, currentSwSuperVisor?.id, currentProvider?.id);
                                    }}
                                />
                            </>
                        );
                        ...
                        return (
                            <>
                                ...
                                <PickySelect
                                    ...
                                    dropdownHeight={600}
                                    getFilterValue={val => {
                                        getDropdownFilterList(val, _.get(searchKeys, passedColumns[3].columnName, 'customer.customerNumber'), [passedColumns[3].columnName], id, currentServiceWorkerFromSuperVisor?.id, currentWard?.id, currentSwSuperVisor?.id, currentProvider?.id);
                                    }}
                                />
                            </>
                        );
                        ...
                        return (
                            <>
                                ...
                                <PickySelect
                                    ...
                                    dropdownHeight={600}
                                    getFilterValue={val => {
                                        getDropdownFilterList(val, _.get(searchKeys, passedColumns[4].columnName, 'ward.label'), [passedColumns[4].columnName], id, currentServiceWorkerFromSuperVisor?.id, currentWard?.id, currentSwSuperVisor?.id, currentProvider?.id);
                                    }}
                                />
                            </>
                        );
                        ...
                        return (
                            <>
                                ...
                                <PickySelect
                                    ...
                                    dropdownHeight={600}
                                    getFilterValue={val => {
                                        getDropdownFilterList(val, _.get(searchKeys, passedColumns[5].columnName, 'customer.address'), [passedColumns[4].columnName], id, currentServiceWorkerFromSuperVisor?.id, currentWard?.id, currentSwSuperVisor?.id, currentProvider?.id);
                                    }}
                                />
                            </>
                        );
                        ...
                        return (
                            <>
                                ...
                                <PickySelect
                                    ...
                                    dropdownHeight={600}
                                    getFilterValue={val => {
                                        getDropdownFilterList(val, _.get(searchKeys, passedColumns[6].columnName, 'customer.associationName'), [passedColumns[6].columnName], id, currentServiceWorkerFromSuperVisor?.id, currentWard?.id, currentSwSuperVisor?.id, currentProvider?.id);
                                    }}
                                />
                            </>
                        );
    ...
    const selectAllPermissionUnder = hasAccessPermission(RESOURCE_MAPPING.ORGANIZATION_ASSIGN_CUSTOMER_TO_SERVICE_WORKER_CUSTOMERS_UNDER_CURRENT_SERVICE_PROVIDER, ACTION_MAPPING.ORGANIZATION_ASSIGN_CUSTOMER_TO_SERVICE_WORKER_CUSTOMERS_UNDER_CURRENT_SERVICE_PROVIDER.SELECT_ALL) || false;
    const selectAllPermissionNotUnder = hasAccessPermission(RESOURCE_MAPPING.ORGANIZATION_ASSIGN_CUSTOMER_TO_SERVICE_WORKER_CUSTOMERS_NOT_UNDER_CURRENT_SERVICE_PROVIDER, ACTION_MAPPING.ORGANIZATION_ASSIGN_CUSTOMER_TO_SERVICE_WORKER_CUSTOMERS_UNDER_CURRENT_SERVICE_PROVIDER.SELECT_ALL) || false;
            ...
            <Form onSubmit={props.handleSubmit(submit)}>
                <Grid container spacing={2}>
                    <Grid item xs={12} sm={12} md={12} spacing={2} className={classes.item}>
                        ...
                        <Field spinnerProps="selectTagProp" name='serviceProviders' label={I18n.t('service_provider')} component={renderSelect}
                            onChange={(resourceItem) => {
                                ...
                                setCurrentProvider(resourceItem);
                                dispatch(Actions.getWardsUnderProviderId(id, resourceItem?.id));

                            }}>
                            {
                                [getEmptyPicky(), ...serviceProviders]
                    ...
                    <Grid item xs={12} sm={12} md={12} spacing={2} className={classes.item}>
                        ...
                        <Field spinnerProps="selectTagProp" name='wards' label={I18n.t('ward')} component={renderSelect}
                            onChange={(resourceItem) => {
                                ...
                                dispatch(Actions.getSwSuperVisorForWard({ wardId: resourceItem.id, orgId: id, providerId: currentProvider?.id }));
                            }}>
                            {
                    ...
                    <Grid item xs={12} sm={12} md={12} className={classes.item}>
                        ...
                        <Field spinnerProps={swSuperVisorForWard?.requestInProgress} name='swSuperVisor' label={I18n.t('sw_superVisor')} component={renderSelect}
                            onChange={(resourceItem) => {
                                ...
                                dispatch(Actions.getServiceWorkerFromSWSuperVisor({ swSuperVisorId: resourceItem.id, wardId: currentWard?.id, orgId: id, providerId: currentProvider?.id }));
                            }}>
                            {
                    ...
                    <Grid item xs={12} sm={12} md={12} className={classes.item}>
                        ...
                        <Field spinnerProps={serviceWorkerFromSwSuperVisor?.requestInProgress} name='serviceWorker' label={I18n.t('service_worker')} component={renderSelect}
                            onChange={(resourceItem) => {
                                ...
                                setMember(true);
                                props.change('unassignedUsersOnly', false);
                                conditionalReload({ unassignedUsersOnly, member: true, organizationId: Number(id), serviceProviderId: Number(currentProvider?.id), wardId: currentWard?.id, gtId: resourceItem?.id, apiCallFlag: true, supervisorId: currentSwSuperVisor?.id });
                            }}>
                            {
                    ...
                    <div className={classes.root}>
                        <Tabs value={value} onChange={handleChange} aria-label="Vertical tabs example" orientation='vertical' style={{ width: '20%', display: 'flex', flexDirection: 'column', alignItems: 'center' }} className={classes.tabs}>
                            {
                                hasAccessPermission(RESOURCE_MAPPING.ORGANIZATION_ASSIGN_CUSTOMER_TO_SERVICE_WORKER_CUSTOMERS_UNDER_CURRENT_SERVICE_PROVIDER, ACTION_MAPPING.ORGANIZATION_ASSIGN_CUSTOMER_TO_SERVICE_WORKER_CUSTOMERS_UNDER_CURRENT_SERVICE_PROVIDER.ACCESS_IN_TAB) && <Tab label={I18n.t('show_customers_under_current_service_provider')} {...a11yProps(0)} onClick={() => onMemberClick(true)} />
                            }
                            {
                                hasAccessPermission(RESOURCE_MAPPING.ORGANIZATION_ASSIGN_CUSTOMER_TO_SERVICE_WORKER_CUSTOMERS_NOT_UNDER_CURRENT_SERVICE_PROVIDER, ACTION_MAPPING.ORGANIZATION_ASSIGN_CUSTOMER_TO_SERVICE_WORKER_CUSTOMERS_NOT_UNDER_CURRENT_SERVICE_PROVIDER.ACCESS_IN_TAB) && <Tab label={I18n.t('show_customers_not_under_current_service_provider')} {...a11yProps(1)} onClick={() => onMemberClick(false)} />
                            }

                        </Tabs>
                        <TabPanel value={value} index={0}>
                            {selectAllPermissionUnder && <Grid item xs={12} sm={12} md={12}>
                                <Field component="input" name='allSelected' type="checkbox" onClick={() => checkBoxClick()} /><span>{I18n.t('all_selected')}</span>
                            </Grid>
                            }
                            {selectAllPermissionNotUnder && <Grid item xs={12} sm={12} md={12}>
                                <Field component="input" name='allSelected' type="checkbox" onClick={() => checkBoxClick()} /><span>{I18n.t('all_selected')}</span>
                            </Grid>
                            }
