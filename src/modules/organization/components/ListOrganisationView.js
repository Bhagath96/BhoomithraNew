...
function ListOrganizationView(props) {
    ...
    useEffect(() => {
        dispatch(Actions.resetTabIndex());
    ...
    const editPressed = (rowData) => {
        localStorage.removeItem('typeOfOrganization');
                    ...
                    display: (filterList, onChange, index, column) => {
                        return (
                            <PickySelect
                                ...
                                includeFilter={true}
                                onChange={value => {
                                    setOrganizationFilter({ [passedColumns[2].apiParam]: { value, property: [passedColumns[2].filterBy] } });
                                    filterList[index] = value.map(val => val.name);
                                    onChange(filterList[index], index, column);
                                }}
                                dropdownHeight={600}
    ...
    const showViewAction = hasAccessPermission(RESOURCE_MAPPING.ORGANIZATION, ACTION_MAPPING.ORGANIZATION.VIEW_IN_ACTION);
    const showActionMenu = hasAccessPermission(RESOURCE_MAPPING.ORGANIZATION, ACTION_MAPPING.ORGANIZATION.EDIT_IN_ACTION) || hasAccessPermission(RESOURCE_MAPPING.ORGANIZATION, ACTION_MAPPING.ORGANIZATION.DELETE_IN_ACTION) || showViewAction || false;
    const showAddIcon = hasAccessPermission(RESOURCE_MAPPING.ORGANIZATION, ACTION_MAPPING.ORGANIZATION.ADD);
    if (showActionMenu) {
        let actions = {
            name: 'Actions',
            ...
            options: {
                ...
                customBodyRender: (value, tableMeta) => {
                    ...
                    if (hasAccessPermission(RESOURCE_MAPPING.ORGANIZATION, ACTION_MAPPING.ORGANIZATION.EDIT_IN_ACTION)) {
                        menuActions.push({ name: I18n.t('edit'), fn: () => editPressed(rowData) });
                    }
                    if (hasAccessPermission(RESOURCE_MAPPING.ORGANIZATION, ACTION_MAPPING.ORGANIZATION.DELETE_IN_ACTION)) {
