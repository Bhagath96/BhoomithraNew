...
const AssignUsersView = (props) => {
    ...
    const readOnlyPermission = hasAccessPermission(RESOURCE_MAPPING.ORGANIZATION_ASSIGN_ORGANIZATION, ACTION_MAPPING.ORGANIZATION_ASSIGN_ORGANIZATION.READ_ONLY);
    ...
    const usersInCurrentOrgPermission = (hasAccessPermission(RESOURCE_MAPPING.ORGANIZATION_ASSIGN_USER, ACTION_MAPPING.ORGANIZATION_ASSIGN_USER.USERS_BELONG_TO_CURRENT_ORGANIZATION) || false);
    const usersNotIncludedInAnyOrgPermission = (hasAccessPermission(RESOURCE_MAPPING.ORGANIZATION_ASSIGN_USER, ACTION_MAPPING.ORGANIZATION_ASSIGN_USER.USERS_DONT_HAVE_ANY_ORGANIZATION_ASSOCIATION) || false);

    return (
        <Form onSubmit={props.handleSubmit(submit)}>
            <Grid container>
                {usersInCurrentOrgPermission && <Grid item xs={12} sm={6} ><Field component="input" name='member' type="checkbox" onClick={onMemberClick} /><span>{I18n.t('users_assigned_in_current_organization')}</span><br /><br /></Grid>}
                {!member && usersNotIncludedInAnyOrgPermission && <Grid item xs={12} sm={6}><Field component="input" name='unassignedUsersOnly' type="checkbox" onClick={onUnassignedUsersOnlyClick} /><span>{I18n.t('show_users_un_assigned_to_any_organizations')}</span><br /><br /></Grid>}
            </Grid>
            <MUIDataTable
