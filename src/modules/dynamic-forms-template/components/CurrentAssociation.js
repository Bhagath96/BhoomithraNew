...
import { fetchCurrentAssociation } from '../actions';
...
function CurrentAssociation() {
    ...
    const { fetchCurrentAssociation: { data: CurrentAssociationList = [], requestInProgress } = {} } = initialValues;
    useEffect(() => {
        dispatch(fetchCurrentAssociation(id));
    }, []);
    return (
        ...
        <div className={classes.root}>
            <Tabs value={value} onChange={handleChange} aria-label="Vertical tabs example" orientation='vertical' style={{ width: '20%', display: 'flex', flexDirection: 'column', alignItems: 'center' }} className={classes.tabs}>
                {
                    (hasAccessPermission(RESOURCE_MAPPING.TEMPLATE_CURRENT_ASSOCIATION_QUESTION, ACTION_MAPPING.TEMPLATE_CURRENT_ASSOCIATION_QUESTION.ACCESS_IN_TAB) || false) &&
                    <Tab label={I18n.t('questions')}{...a11yProps(0)} />
                }
                {
                    (hasAccessPermission(RESOURCE_MAPPING.TEMPLATE_CURRENT_ASSOCIATION_TITLE, ACTION_MAPPING.TEMPLATE_CURRENT_ASSOCIATION_TITLE.ACCESS_IN_TAB) || false) &&
                    <Tab label={I18n.t('titles')} {...a11yProps(1)} />
                }
                {
                    (hasAccessPermission(RESOURCE_MAPPING.TEMPLATE_CURRENT_ASSOCIATION_FRAGMENT, ACTION_MAPPING.TEMPLATE_CURRENT_ASSOCIATION_FRAGMENT.ACCESS_IN_TAB) || false) &&
                    <Tab label={I18n.t('fragments')} {...a11yProps(2)} />
                }
            </Tabs>
