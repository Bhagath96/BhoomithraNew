...
import { fetchCurrentAssociationQuestion } from '../actions';
...
function CurrentAssociationQuestion() {
    const columns = [
        {
            name: 'id',
            label: I18n.t('id')
    ...
    const { CurrentAssociationQuestion: { data: CurrentAssociationList = [], requestInProgress } = {} } = initialValues;

    useEffect(() => {
        dispatch(fetchCurrentAssociationQuestion(id));
    }, []);
    ...
    return (
        ...
        <div className={classes.root}>
            <Tabs value={value} onChange={handleChange} aria-label="Vertical tabs example" orientation='vertical' style={{ width: '20%', display: 'flex', flexDirection: 'column', alignItems: 'center' }} className={classes.tabs}>
                {
                    (hasAccessPermission(RESOURCE_MAPPING.QUESTION_CURRENT_ASSOCIATION_FRAGMENT, ACTION_MAPPING.QUESTION_CURRENT_ASSOCIATION_FRAGMENT.ACCESS_IN_TAB) || false) &&

                    <Tab label={I18n.t('fragments')} {...a11yProps(0)} />
                }
                {
                    (hasAccessPermission(RESOURCE_MAPPING.QUESTION_CURRENT_ASSOCIATION_TEMPLATE, ACTION_MAPPING.QUESTION_CURRENT_ASSOCIATION_TEMPLATE.ACCESS_IN_TAB) || false) &&
                    <Tab label={I18n.t('templates')} {...a11yProps(1)} />
                }
                {
                    (hasAccessPermission(RESOURCE_MAPPING.QUESTION_CURRENT_ASSOCIATION_TITLE, ACTION_MAPPING.QUESTION_CURRENT_ASSOCIATION_TITLE.ACCESS_IN_TAB) || false) &&
                    <Tab label={I18n.t('titles')} {...a11yProps(2)} />
                }
...
export default CurrentAssociationQuestion;
