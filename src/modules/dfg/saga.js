import { all, takeLatest, call, fork, spawn, put, select, delay, race, take } from 'redux-saga/effects';
...
import { getDefaultOrganization, hasGtRole, infoToast, successToast, errorToast, getPayloadData } from './common/utils';
import {
...
const { surveyDataLock, syncInprogressDataLock, fetchSurveyTemplateLock, forceUpdateSurveyTemplateLock } = locks;
const { SurveyDataRepository } = Repositories;

function* handleFetchSurveyTemplates() {
    yield call([fetchSurveyTemplateLock, 'acquireAsync']);
    // metadata using only to get languageSwitched and templatesLastUpdated
    ...
    templatesLastUpdated = templatesLastUpdated ? moment(templatesLastUpdated) : moment();
    let templates = yield call([Storage, 'getItem'], SURVEY_TEMPLATE_STORE_KEY);
    templates = templates ? JSON.parse(templates) : {};
    // Force update templates if previous update was not done on the same day || language switched
    if (_.isEmpty(templates) || metadata.languageSwitched || moment().isAfter(templatesLastUpdated, 'day')) {
        yield call(forceUpdateSurveyTemplates);
        if (metadata.languageSwitched) {
            metadata.languageSwitched = false;
    ...
    if (surveyTemplateApiAction.type === ActionTypes.SURVEY_TEMPLATE_API_SUCCESS) {
        ...
        if (!_.isEmpty(surveyTemplateApiResponse)) {
            if (Object.keys(surveyTemplateApiResponse).filter(currentKey => !excludedKeys.includes(currentKey)).length > 0) {
                yield call(successToast, I18n.t('survey_template_updated_successfully'));
            }
        }
    }
    yield call([fetchSurveyTemplateLock, 'release']);
...
function* forceUpdateSurveyTemplates() {
    yield call([forceUpdateSurveyTemplateLock, 'acquireAsync']);
    yield call(infoToast, I18n.t('updating_all_templates'), 3000);
    const userInfo = yield select(getUserInfo);
    const { id: userId } = userInfo;
    const defaultOrganization = getDefaultOrganization(userInfo);
    if (!defaultOrganization) {
        yield call(errorToast, I18n.t('organizations_unavailable'));
        yield call([forceUpdateSurveyTemplateLock, 'release']);
        return;
    }
    const surveyTemplateApiRequest = {
        orgId: defaultOrganization.id,
    ...
    yield fork(saga.handleAPIRequest, DfgAPI.fetchSurveyTemplate, surveyTemplateApiRequest, false);
    const surveyTemplateApiAction = yield take([ActionTypes.SURVEY_TEMPLATE_API_SUCCESS, ActionTypes.SURVEY_TEMPLATE_API_FAILED]);
    if (surveyTemplateApiAction.type === ActionTypes.SURVEY_TEMPLATE_API_SUCCESS) {
        let metadata = yield call([Storage, 'getItem'], METADATA_STORE_KEY);
        metadata = metadata ? JSON.parse(metadata) : {};
        ...
        yield call([Storage, 'setItem'], METADATA_STORE_KEY, JSON.stringify(metadata));
        const surveyTemplateApiResponse = getPayloadData(surveyTemplateApiAction.payload.data);
        let mergedTemplates = _.merge({}, surveyTemplateApiResponse);
        yield call([Storage, 'setItem'], SURVEY_TEMPLATE_STORE_KEY, JSON.stringify(mergedTemplates));
        yield call(successToast, I18n.t('survey_template_updated_successfully'));
    } else if (surveyTemplateApiAction.type === ActionTypes.SURVEY_TEMPLATE_API_FAILED) {
        yield call(errorToast, I18n.t('survey_templates_update_failed'));
    }
    yield call([forceUpdateSurveyTemplateLock, 'release']);
}
...
function* generateDynamicTemplate(render, values) {
    ...
    let template = templates[surveyData.version];
...
function* processConnectedQuestions(action) {
    ...
    if (optionsHavingConnectedQuestions[question].includes(value)) {
        ...
        connectedQuestionsToShow.push(...connectedQuestionsToAdd);
...
function* loadCompletedSurveys(action) {
    const initializer = yield select(getInitializer);
    const { templateTypeIds, uiKeys } = initializer;
...
export default function* dfgSaga() {
    yield all([
        ...
        takeLatest(ActionTypes.FORCE_UPDATE_SURVEY_TEMPLATES, forceUpdateSurveyTemplates),
        ...
        takeLatest(ActionTypes.SYNC_INPROGRESS_DATA, syncInprogressData),
...
export {
    loadIncompleteSurveys, removeInprogressData, postProcessSurveyData,
