...
function ListUserGroupView(props) {
    ...
    const handleClick = () => {
        history.push('/admin/index/user-group/create');
    };

    const editPressed = (rowData) => {
        let id = rowData[0];
        // dispatch(setUserGroup(userGroupId));
        history.push(`/admin/index/user-group/${id}/details`);
    };
                        ...
                        return (
                            <PickySelect
                                ...
                                onChange={(value) => {
                                    ...
                                    setUserGroupFilter({ [passedColumns[2].apiParam]: { value, property: [passedColumns[2].filterBy] } });
                                    filterList[index] = value.map(val => val.name);
                                    onChange(filterList[index], index, column);
                                }}
                                dropdownHeight={600}
                                getFilterValue={val => {
                                    getDropdownFilterList(val, searchKeys['usergroup.description'], [passedColumns[2].columnName]);
    ...
    const showActionMenu = hasAccessPermission(RESOURCE_MAPPING.USER_GROUP, ACTION_MAPPING.USER_GROUP.EDIT_IN_ACTION) || hasAccessPermission(RESOURCE_MAPPING.USER_GROUP, ACTION_MAPPING.USER_GROUP.DELETE_IN_ACTION) || false;
    const showAddIcon = hasAccessPermission(RESOURCE_MAPPING.USER_GROUP, ACTION_MAPPING.USER_GROUP.ADD);
    if (showActionMenu) {
        let actions = {
            name: 'Actions',
            ...
            options: {
                ...
                customBodyRender: (value, tableMeta) => {
                    ...
                    let menuActions = [];
                    if (hasAccessPermission(RESOURCE_MAPPING.USER_GROUP, ACTION_MAPPING.USER_GROUP.EDIT_IN_ACTION)) {
                        menuActions.push({ name: I18n.t('edit'), fn: () => editPressed(rowData) });
                    }
                    if (hasAccessPermission(RESOURCE_MAPPING.USER_GROUP, ACTION_MAPPING.USER_GROUP.DELETE_IN_ACTION)) {
