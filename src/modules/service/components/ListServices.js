...
const { MUIDataTable, PickySelect, Typography, Checkbox } = Components;
...
import { AccessTimeTwoTone, Check } from '@material-ui/icons';
import { convertToLocal } from '../../../utils/DateUtils';

function ListServices(props) {

    const dispatch = useDispatch();
    const { listServices: { requestInProgress = false, data: { content = [] } = {}, searchKeys } } = props;
    const { getDropdownFilterList, setServiceFilter, setPassedColumnsForTable, setPagination, setChips, setPageProps, loadServices } = props;
    ...
    const handleClick = (e) => {
        dispatch(fetchServiceDetails(e.target.checked || false));
    };
                        ...
                        return (
                            <>
                                ...
                                <PickySelect
                                    ...
                                    includeFilter={true}
                                    onChange={value => {
                                        setServiceFilter({ [passedColumns[5].apiParam]: { value, property: [passedColumns[5].filterBy] } });
                                        filterList[index] = value.map(val => val.name);
                                        onChange(filterList[index], index, column);
                                    }}
                                    dropdownHeight={600}
                                    getFilterValue={val => {
                                        getDropdownFilterList(val, _.get(searchKeys, 'user.serviceWorkerName'), [passedColumns[4].columnName]);
                        ...
                        return (
                            <>
                                ...
                                <PickySelect
                                    ...
                                    getFilterValue={val => {
                                        if (val.length > 1) {
                                            getDropdownFilterList(val, _.get(searchKeys, 'm_service_config_label.serviceName'), [passedColumns[7].columnName]);
                                        }
                                    }}
                                />
                            </>
                        ...
                        return (
                            <>
                                ...
                                <PickySelect
                                    ...
                                    getFilterValue={val => {
                                        if (val.length > 1) {
                                            getDropdownFilterList(val, _.get(searchKeys, 'user.supervisorName'), [passedColumns[8].columnName]);
                                        }
                                    }}
                                />
                            </>
                        ...
                        return (
                            <>
                                ...
                                <PickySelect
                                    ...
                                    includeFilter={true}
                                    onChange={value => {
                                        setServiceFilter({ [passedColumns[9].apiParam]: { value, property: [passedColumns[9].filterBy] } });
                                        filterList[index] = value.map(val => val.name);
                                        onChange(filterList[index], index, column);
                                    }}
                                    dropdownHeight={600}
                                    getFilterValue={val => {
                                        if (val.length > 1) {
                                            getDropdownFilterList(val, _.get(searchKeys, 'organization_service_provider.serviceProviderName'), [passedColumns[9].columnName]);
                                        }
                                    }}
                                />
                            </>
                        ...
                        return (
                            <>
                                ...
                                <PickySelect
                                    ...
                                    includeFilter={true}
                                    onChange={value => {
                                        setServiceFilter({ [passedColumns[10].apiParam]: { value, property: [passedColumns[10].filterBy] } });
                                        filterList[index] = value.map(val => val.name);
                                        onChange(filterList[index], index, column);
                                    }}
                                    dropdownHeight={600}
                                    getFilterValue={val => {
                                        if (val.length > 1) {
                                            getDropdownFilterList(val, _.get(searchKeys, 'm_residence_category_label.label'), [passedColumns[10].columnName]);
                                        }
                                    }}
                                />
                            </>
    ...
    // const showActionMenu = hasAccessPermission(RESOURCE_MAPPING.SERVICE, ACTION_MAPPING.SERVICE.LIST_SERVICE_HISTORY) || false;
    // if (showActionMenu) {
    //     let actions = {
    //         name: 'Actions',
    ...
    const options = {
        ...
        count,
        customActions: [
            { handleClick: handleClick, icon: < Checkbox />, Typography: I18n.t('skipped') },
            { icon: < Typography>{I18n.t('skipped')}</Typography> }
        ],
        ...
        onFilterDialogOpen: () => dispatch(setFilterValuesFromInitialStates({ key: TABLE_IDS.LIST_SERVICE })),
...
const mapDispatchToProps = dispatch => ({
    getDropdownFilterList: (searchValue, searchKey, columnName) => {
        if (searchValue && searchValue?.length > 2) {
            dispatch(fetchTableDropdownFilterList({ searchValue, searchKey, columnName, key: TABLE_IDS.LIST_SERVICE, url: URL.LIST_SERVICE.LIST_SERVICE }));
        }
        if (searchValue === '') {
            dispatch(setFilterValuesFromInitialStates({ key: TABLE_IDS.LIST_SERVICE }));
        }
    ...
    loadServices: (data) => dispatch(fetchServiceDetails(data)),
    setPageProps: (data) => {
