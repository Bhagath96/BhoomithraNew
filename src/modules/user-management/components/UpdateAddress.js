...
import { loadStates } from '../../common/actions';

import { getAddressOfTheUser, UpdateAddResss, loadDistricts } from '../actions';
...
function UpdateAddress(props) {
    ...
    useEffect(() => {
        dispatch(loadStates({ identifier: 'User' }));
    ...
    const PermenentstateValue = useSelector(state => _.get(state, 'form.UpdateAddress.values.permanentAddress.state', {}));
    const currentstateValue = useSelector(state => _.get(state, 'form.UpdateAddress.values.currentAddress.state', {}));
    // const [defaultAddress, setDefaultAddress] = useState(1);
    const { id: stateId } = PermenentstateValue;
    const { id: currentAddressStateId } = currentstateValue;

    useEffect(() => {
        if (stateId) {
            dispatch(loadDistricts({ identifier: 'PR', id: { stateId: stateId } }));
        }
        if (currentAddressStateId) {
    ...
    const submit = (values = {}) => {
        ...
        dispatch(UpdateAddResss(id, newAddressArray));
    };
