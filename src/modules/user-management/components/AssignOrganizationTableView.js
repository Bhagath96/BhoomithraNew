...
const AssignOrganizationUnderUserView = (props) => {
    ...
    const { assignOrganisationList: { data: defaultOrgList = [] }, assignOrganizationForUser: { data: assignOrgUserData = [], defaultOrganization = {}, requestInProgress = false, searchKeys = [] } = {} } = useSelector(state => _.get(state, STATE_REDUCER_KEY, {}));
    ...
    useEffect(() => {
        change('defaultOrganization', defaultOrganization);
    }, [assignOrgUserData?.content]);
    ...
    const submit = (values) => {
        let defaultOrgId = _.get(values, 'defaultOrganization.id', null);
        let assignOrgObj = {
            organizationIds: selectedIds,
            member: member,
            defaultOrganizationId: defaultOrgId
        };
        dispatch(Actions.submitAssignOrganizationUnderUser({ assignOrgObj, userId: id }));
        resetForm();
    };
    ...
    const onUnassignedUsersOnlyClick = event => {
        ...
        dispatch(setAdditionalDropDownFilters({ key: TABLE_IDS.LIST_ASSIGN_ORGANIZATION_UNDER_USER, additionalDropdownFilters: { member: member, unassignedRolesOnly: flag } }));
    ...
    const onMemberClick = event => {
        ...
        dispatch(setAdditionalDropDownFilters({ key: TABLE_IDS.LIST_ASSIGN_ORGANIZATION_UNDER_USER, additionalDropdownFilters: { member: flag, unassignedRolesOnly: unassignedOrgOnly } }));

        conditionalReload({
            unassignedOrgOnly, member: flag, userId: id
        });
    };
    const orgInCurrentUserPermission = (hasAccessPermission(RESOURCE_MAPPING.USER_ASSIGN_ORGANIZATION, ACTION_MAPPING.USER_ASSIGN_ORGANIZATION.ORGANIZATIONS_BELONG_TO_CURRENT_USER) || false);
    const orgNotIncludedInAnyUserPermission = (hasAccessPermission(RESOURCE_MAPPING.USER_ASSIGN_ORGANIZATION, ACTION_MAPPING.USER_ASSIGN_ORGANIZATION.ORGANIZATIONS_DONT_HAVE_ANY_USER_ASSOCIATION) || false);
    const defaultOrgPermission = (hasAccessPermission(RESOURCE_MAPPING.USER_ASSIGN_ORGANIZATION, ACTION_MAPPING.USER_ASSIGN_ORGANIZATION.DEFAULT_ORGANIZATION) || false);
    return (
        <Form onSubmit={props.handleSubmit(submit)}>
            <Grid container>
                {orgInCurrentUserPermission && <Grid item xs={12} sm={6} ><Field component="input" name='member' type="checkbox" onClick={onMemberClick} /><span>{I18n.t('organization_belongs_to_the_current_user')}</span><br /><br /></Grid>}
                {!member && orgNotIncludedInAnyUserPermission && <Grid item xs={12} sm={6}><Field component="input" name='unassignedOrgOnly' type="checkbox" onClick={onUnassignedUsersOnlyClick} /><span>{I18n.t('organization_dont_have_any_user_association')}</span><br /><br /></Grid>}
            </Grid>
            <MUIDataTable
            ...
            />
            <br></br>
            {defaultOrgPermission && <Grid item xs={12} sm={12} >
                <Field spinnerProps="selectTagProp" showLoader={requestInProgress} name='defaultOrganization' label={I18n.t('default_organization')} component={renderSelect}
                    onChange={(values) => {
                        change('defaultOrganizationId', values.id);
                    }}
                >
                    {
                        [getEmptyPicky(), ...defaultOrgList]
...
const mapDispatchToProps = dispatch => ({
    ...
    resetTableFilterList: ({ userId }) => {
        ...
        dispatch(setAdditionalDropDownFilters({ key: TABLE_IDS.LIST_ASSIGN_ORGANIZATION_UNDER_USER, additionalDropdownFilters: { member: false, unassignedRolesOnly: false } }));
    },
    setPageProps: (data) => {
        dispatch(resetTableDropdownFilterList({ key: TABLE_IDS.LIST_ASSIGN_ORGANIZATION_UNDER_USER }));
