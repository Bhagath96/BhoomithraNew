...
import { getDataAccessPermissionUserRole } from '../data-access-permission/selectors';
...
function* listUsers() {
    ...
    let userTableData = getPayloadData(userAction.payload, {});
...
function* getAddressOfUser(action) {
    yield call(saga.handleAPIRequest, API.getAddressOfUser, action.payload);
}
...
function* listDistricts(action) {
    ...
    let types = [ActionTypes.LIST_DISTRICTS_FOR_ADDRESS_REQUEST, ActionTypes.LIST_DISTRICTS_FOR_ADDRESS_SUCCESS, ActionTypes.LIST_DISTRICTS_FOR_ADDRESS_FAILURE];
    let body = { stateId };
    yield fork(saga.handleAPIRequest, CommonAPI.loadDistricts, { types, body });
...
function* submitAssignOrganizationForUsers(action) {
    let { userId, assignOrgObj } = action.payload.data;
    ...
    if (updateAssignOrganizationAction.type === ActionTypes.SUBMIT_ASSIGN_ORGANIZATION_UNDER_USER_SUCCESS) {
        ...
        yield call(successNotify, I18n.t(_.get(assignOrgObj, 'member', false) === false ? 'assigned_success' : 'un_assigned_success', { type: I18n.t('organization') }));
...
function* submitUserRoleDataAccessAssignment(action) {
    let { payload: { data: { userId = null } = {} } = {} } = action;
    const accessPermissions = yield select(getDataAccessPermissionUserRole);
    let dataAccessRequest = [];
    _.forEach(_.get(accessPermissions, 'data', []), dataAccess => {
        let roleId = _.get(dataAccess, 'details.id', null);
    ...
    yield fork(saga.handleAPIRequest, API.submitUserRoleDataAccess, { dataAccessRequest, userId });
    const saveRoleAssigneeAccessPermissionAction = yield take([ActionTypes.SUBMIT_USER_ROLE_DATA_ACCESS_SUCCESS, ActionTypes.SUBMIT_USER_ROLE_DATA_ACCESS_FAILURE]);
    if (saveRoleAssigneeAccessPermissionAction.type === ActionTypes.SUBMIT_USER_ROLE_DATA_ACCESS_SUCCESS) {
...
export default function* userSaga() {
    yield all([
        ...
        takeLatest(ActionTypes.SUBMIT_ASSIGN_ROLE_UNDER_USER, submitAssignRoleUnderUser),
        ...
        takeEvery(ActionTypes.LIST_GENDER, listGender),
        takeLatest(ActionTypes.SUBMIT_USER_ROLE_DATA_ACCESS, submitUserRoleDataAccessAssignment)
    ]);
}
