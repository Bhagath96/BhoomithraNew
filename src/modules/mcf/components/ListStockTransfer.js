...
import { fetchStockTransferDetails } from '../actions';
...
import { getStockTransfer } from '../selectors';
...
function ListStockIn(props) {
    ...
    const { tableProps: { [TABLE_IDS.LIST_STOCK_TRANSFER]: { filterState = {}, filterOptions = {}, pagination: { page, size, count } = DEFAULT_TABLE_PROPS } = {} } } = useSelector(state => state.common);
    ...
    const passedColumns = [
        ...
        { columnLabel: 'transfer_to', columnName: 'transferTo.name', apiParam: 'transferredTo', filterBy: 'id' },
        { columnLabel: 'facility_type', columnName: 'facilityType.name', apiParam: 'transactedBy.name', filterBy: 'id' }
    ...
    useEffect(() => {
        ...
        setPagination(DEFAULT_TABLE_PROPS);
        dispatch(resetFilter({ key: TABLE_IDS.LIST_STOCK_TRANSFER }));
        setChips({});
        loadStockIn();
    }, []);
                        ...
                        return (
                            <>
                                ...
                                <PickySelect
                                    ...
                                    dropdownHeight={600}
                                    getFilterValue={val => {
                                        getDropdownFilterList(val, _.get(searchKeys, 'mcf_rrf_ckc_sale.transferToName'), [passedColumns[3].columnName]);
                                    }}
                                />
                            </>
                        );
                        ...
                        return (
                            <>
                                ...
                                <PickySelect
                                    ...
                                    dropdownHeight={600}
                                    getFilterValue={val => {
                                        getDropdownFilterList(val, _.get(searchKeys, 'state.code'), [passedColumns[4].columnName]);
                                    }}
                                />
                            </>
                        );
    ...
    const viewPressed = (id) => {
        history.push(`${PATH.MCF_STOCK_TRANSFER}/${id}/view`);
    };
    const showActionMenu = hasAccessPermission(RESOURCE_MAPPING.MCF, ACTION_MAPPING.MCF.ACCESS_MCF_STOCK_TRANSFER_IN_WEB_NAV) || true;
    if (showActionMenu) {
        let actions = {
            name: 'Actions',
    ...
    const options = {
        ...
        onFilterDialogOpen: () => dispatch(setFilterValuesFromInitialStates({ key: TABLE_IDS.LIST_STOCK_TRANSFER })),
...
const mapStateToProps = createStructuredSelector({
    listStockes: getStockTransfer,
    tableProps: getTableProps
});
const mapDispatchToProps = dispatch => ({
    getDropdownFilterList: (searchValue, searchKey, columnName) => {
        if (searchValue && searchValue?.length > 2) {
            dispatch(fetchTableDropdownFilterList({ searchValue, searchKey, columnName, key: TABLE_IDS.LIST_STOCK_TRANSFER, url: URL.MCF.GET_ALL_MCF_STOCKS_TRANSFER }));
        }
        if (searchValue === '') {
            dispatch(setFilterValuesFromInitialStates({ key: TABLE_IDS.LIST_STOCK_TRANSFER }));
        }
    },
    setPassedColumnsForTable: (data) => dispatch(setPassedColumns({ key: TABLE_IDS.LIST_STOCK_TRANSFER, passedColumns: data })),
    setStateFilter: (data) => {
        dispatch(setTableFilterState({ key: TABLE_IDS.LIST_STOCK_TRANSFER, filterState: data }));
    },
    loadStockIn: (data) => dispatch(fetchStockTransferDetails(data)),
    setPageProps: (data) => {
        dispatch(resetTableDropdownFilterList({ key: TABLE_IDS.LIST_STOCK_TRANSFER }));
        dispatch(setTablePagination({ key: TABLE_IDS.LIST_STOCK_TRANSFER, pagination: data }));
        dispatch(fetchStockTransferDetails());
    },
    setChips: (data) => dispatch(setTableFilterChips({ key: TABLE_IDS.LIST_STOCK_TRANSFER, chips: data })),
    setPagination: (data) => dispatch(setTablePagination({ key: TABLE_IDS.LIST_STOCK_TRANSFER, pagination: data }))
});
