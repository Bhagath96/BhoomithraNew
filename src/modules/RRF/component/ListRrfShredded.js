...
import { fetchRrfShreddedDetails } from '../actions';
...
import { getShredding } from '../selectors';
...
function ListRRFShredded(props) {

    const dispatch = useDispatch();
    // const { listRRFShreddedDetails: { requestInProgress = false, data: { content = [] } = {}, searchKeys } } = useSelector(state => state[STATE_REDUCER_KEY]);
    ...
    const { listShredding: { requestInProgress = false, data: { content = [] } = {}, searchKeys } } = props;

    const { tableProps: { [TABLE_IDS.LIST_RRF_SHREDDED]: { filterState = {}, filterOptions = {}, pagination: { page, size, count } = DEFAULT_TABLE_PROPS } = {} } } = useSelector(state => state.common);
    ...
    const passedColumns = [
        ...
        { columnLabel: 'transacted_by', columnName: 'transactedBy.name', apiParam: 'transactedBy', filterBy: 'id' },
        { columnLabel: 'quantity_in_kg', columnName: 'quantityInKg', apiParam: 'quantities', filterBy: 'id' }
    ...
    useEffect(() => {
        ...
        setPagination(DEFAULT_TABLE_PROPS);
        dispatch(resetFilter({ key: TABLE_IDS.LIST_RRF_SHREDDED }));
        setChips({});
        loadStockIn();
    }, []);
    ...
    const options = {
        ...
        onFilterDialogOpen: () => dispatch(setFilterValuesFromInitialStates({ key: TABLE_IDS.LIST_RRF_SHREDDED })),
...
const mapStateToProps = createStructuredSelector({
    listShredding: getShredding,
    tableProps: getTableProps
});
const mapDispatchToProps = dispatch => ({
    getDropdownFilterList: (searchValue, searchKey, columnName) => {
        if (searchValue && searchValue?.length > 2) {
            dispatch(fetchTableDropdownFilterList({ searchValue, searchKey, columnName, key: TABLE_IDS.LIST_RRF_SHREDDED, url: URL.MCF.GET_ALL_MCF_STOCKS_IN }));
        }
        if (searchValue === '') {
            dispatch(setFilterValuesFromInitialStates({ key: TABLE_IDS.LIST_RRF_SHREDDED }));
        }
    },
    setPassedColumnsForTable: (data) => dispatch(setPassedColumns({ key: TABLE_IDS.LIST_RRF_SHREDDED, passedColumns: data })),
    setStateFilter: (data) => {
        dispatch(setTableFilterState({ key: TABLE_IDS.LIST_RRF_SHREDDED, filterState: data }));
    },
    loadStockIn: (data) => dispatch(fetchRrfShreddedDetails(data)),
    setPageProps: (data) => {
        dispatch(resetTableDropdownFilterList({ key: TABLE_IDS.LIST_RRF_SHREDDED }));
        dispatch(setTablePagination({ key: TABLE_IDS.LIST_RRF_SHREDDED, pagination: data }));
        dispatch(fetchRrfShreddedDetails());
    },
    setChips: (data) => dispatch(setTableFilterChips({ key: TABLE_IDS.LIST_RRF_SHREDDED, chips: data })),
    setPagination: (data) => dispatch(setTablePagination({ key: TABLE_IDS.LIST_RRF_SHREDDED, pagination: data }))
});
export default connect(mapStateToProps, mapDispatchToProps)(ListRRFShredded);
