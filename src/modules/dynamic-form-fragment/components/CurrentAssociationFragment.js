...
import { fetchFragmentCurrentAssociation } from '../actions';
...
const useStyles = makeStyles((theme) => ({
    ...
    tabs: {
        backgroundColor: Colors['tab-color'],
        // color: 'black',
...
function CurrentAssociationFragment() {
    const columns = [
        {
            name: 'id',
            label: I18n.t('id')
    ...
    const { getFragmentCurrentAssociation: { data: CurrentAssociationList = [], requestInProgress } = {} } = initialValues;
    useEffect(() => {
        dispatch(fetchFragmentCurrentAssociation(id));
    }, []);
    ...
    return (
        ...
        < div className={classes.root} >
            <Tabs value={value} onChange={handleChange} aria-label="Vertical tabs example" orientation='vertical' style={{ width: '20%', display: 'flex', flexDirection: 'column', alignItems: 'center' }} className={classes.tabs}>
                {
                    (hasAccessPermission(RESOURCE_MAPPING.FRAGMENT_CURRENT_ASSOCIATION_QUESTION, ACTION_MAPPING.FRAGMENT_CURRENT_ASSOCIATION_QUESTION.ACCESS_IN_TAB) || false) &&
                    <Tab label={I18n.t('questions')} {...a11yProps(0)} />
                }
                {
                    (hasAccessPermission(RESOURCE_MAPPING.FRAGMENT_CURRENT_ASSOCIATION_TITLE, ACTION_MAPPING.FRAGMENT_CURRENT_ASSOCIATION_TITLE.ACCESS_IN_TAB) || false) &&
                    <Tab label={I18n.t('titles')} {...a11yProps(1)} />
                }
                {
                    (hasAccessPermission(RESOURCE_MAPPING.FRAGMENT_CURRENT_ASSOCIATION_TEMPLATE, ACTION_MAPPING.FRAGMENT_CURRENT_ASSOCIATION_TEMPLATE.ACCESS_IN_TAB) || false) &&
            ...
            <LoadingOverlay active={requestInProgress} >
                ...
                <TabPanel value={value} index={1} className={classes.tabWidth}>
                    <MUIDataTable
                        title=''
                        data={getTableList(CurrentAssociationList, 'titles')}
                        columns={columns}
                        options={options}
                        tabView={{ tabStatus: true }}
                    />
...
export default CurrentAssociationFragment;
