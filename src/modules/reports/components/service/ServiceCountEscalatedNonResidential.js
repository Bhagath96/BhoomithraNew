...
function ServiceCountEscalatedNonResidentialReport() {
    ...
    let report = {
        head: getReportInfo('service_count_escalated_non_residential'),
    ...
    let ExportArray = [];
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_SERVICE_COUNT_ESCALATED_NON_RESIDENTIAL, ACTION_MAPPING.REPORT_SERVICE_COUNT_ESCALATED_NON_RESIDENTIAL.TABLE_COLUMN)) {
        viewColumnFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_SERVICE_COUNT_ESCALATED_NON_RESIDENTIAL, ACTION_MAPPING.REPORT_SERVICE_COUNT_ESCALATED_NON_RESIDENTIAL.TABLE_FILTER)) {
        filterFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_SERVICE_COUNT_ESCALATED_NON_RESIDENTIAL_EXPORT, ACTION_MAPPING.REPORT_SERVICE_COUNT_ESCALATED_NON_RESIDENTIAL_EXPORT.PDF)) {
        report.pdf = getPdfTableData({ columns, data: data2 });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_SERVICE_COUNT_ESCALATED_NON_RESIDENTIAL_EXPORT, ACTION_MAPPING.REPORT_SERVICE_COUNT_ESCALATED_NON_RESIDENTIAL_EXPORT.EXCEL)) {
        report.excel = getExcelTableData({ columns, data: data2 });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_SERVICE_COUNT_ESCALATED_NON_RESIDENTIAL, ACTION_MAPPING.REPORT_SERVICE_COUNT_ESCALATED_NON_RESIDENTIAL.TABLE_EXPORT)) {
    ...
    return (
        <div>
            ...
            <MUIDataTable
                title={I18n.t('service_count_escalated_non_residential')}
                data={data2}
                columns={columns}
                options={options}
            />
...
export default reduxForm({
    form: 'ServiceCountEscalatedNonResidentialReport'
})(ServiceCountEscalatedNonResidentialReport);
