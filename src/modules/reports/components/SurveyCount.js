...
function SurveyCount() {
    ...
    let report = {
        head: getReportInfo('survey_count'),
    ...
    let ExportArray = [];
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_SURVEY_COUNT, ACTION_MAPPING.REPORT_SURVEY_COUNT.TABLE_COLUMN)) {
        viewColumnFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_SURVEY_COUNT, ACTION_MAPPING.REPORT_SURVEY_COUNT.TABLE_FILTER)) {
        filterFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_SURVEY_COUNT_EXPORT, ACTION_MAPPING.REPORT_SURVEY_COUNT_EXPORT.PDF)) {
        report.pdf = getPdfTableData({ columns, data: data2 });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_SURVEY_COUNT_EXPORT, ACTION_MAPPING.REPORT_SURVEY_COUNT_EXPORT.EXCEL)) {
        report.excel = getExcelTableData({ columns, data: data2 });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_SURVEY_COUNT, ACTION_MAPPING.REPORT_SURVEY_COUNT.TABLE_EXPORT)) {
