...
function AdvanceStatus() {
    ...
    let report = {
        head: getReportInfo('advance_status'),
    ...
    let ExportArray = [];
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ADVANCE_STATUS, ACTION_MAPPING.REPORT_ADVANCE_STATUS.TABLE_COLUMN)) {
        viewColumnFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ADVANCE_STATUS, ACTION_MAPPING.REPORT_ADVANCE_STATUS.TABLE_FILTER)) {
        filterFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ADVANCE_STATUS_EXPORT, ACTION_MAPPING.REPORT_ADVANCE_STATUS_EXPORT.PDF)) {
        report.pdf = getPdfTableData({ columns, data: data2 });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ADVANCE_STATUS_EXPORT, ACTION_MAPPING.REPORT_ADVANCE_STATUS_EXPORT.EXCEL)) {
        report.excel = getExcelTableData({ columns, data: data2 });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ADVANCE_STATUS, ACTION_MAPPING.REPORT_ADVANCE_STATUS.TABLE_EXPORT)) {
