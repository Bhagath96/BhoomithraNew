...
function ResidentialGlass() {
    ...
    let report = {
        head: getReportInfo('residential_glass'),
    ...
    let ExportArray = [];
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_GLASS_RESIDENTIAL, ACTION_MAPPING.REPORT_GLASS_RESIDENTIAL.TABLE_COLUMN)) {
        viewColumnFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_GLASS_RESIDENTIAL, ACTION_MAPPING.REPORT_GLASS_RESIDENTIAL.TABLE_FILTER)) {
        filterFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_GLASS_RESIDENTIAL_EXPORT, ACTION_MAPPING.REPORT_GLASS_RESIDENTIAL_EXPORT.PDF)) {
        report.pdf = getPdfTableData({ columns, data: data });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_GLASS_RESIDENTIAL_EXPORT, ACTION_MAPPING.REPORT_GLASS_RESIDENTIAL_EXPORT.EXCEL)) {
        report.excel = getExcelTableData({ columns, data: data });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_GLASS_RESIDENTIAL, ACTION_MAPPING.REPORT_GLASS_RESIDENTIAL.TABLE_EXPORT)) {
    ...
    return (
        <div>
            ...
            <MUIDataTable
                title={I18n.t('residential_glass')}
                data={data}
                columns={columns}
                options={options}
            />
