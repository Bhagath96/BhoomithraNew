...
function ResidentialLeather() {
    ...
    let report = {
        head: getReportInfo('residential_leather'),
    ...
    let ExportArray = [];
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_LEATHER_RESIDENTIAL, ACTION_MAPPING.REPORT_LEATHER_RESIDENTIAL.TABLE_COLUMN)) {
        viewColumnFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_LEATHER_RESIDENTIAL, ACTION_MAPPING.REPORT_LEATHER_RESIDENTIAL.TABLE_FILTER)) {
        filterFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_LEATHER_RESIDENTIAL_EXPORT, ACTION_MAPPING.REPORT_LEATHER_RESIDENTIAL_EXPORT.PDF)) {
        report.pdf = getPdfTableData({ columns, data: data });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_LEATHER_RESIDENTIAL_EXPORT, ACTION_MAPPING.REPORT_LEATHER_RESIDENTIAL_EXPORT.EXCEL)) {
        report.excel = getExcelTableData({ columns, data: data });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_LEATHER_RESIDENTIAL, ACTION_MAPPING.REPORT_LEATHER_RESIDENTIAL.TABLE_EXPORT)) {
    ...
    return (
        <div>
            ...
            <MUIDataTable
                title={I18n.t('residential_leather')}
                data={data}
                columns={columns}
                options={options}
            />
