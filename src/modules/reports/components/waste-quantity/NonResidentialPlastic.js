...
function NonResidentialPlastic() {
    ...
    let report = {
        head: getReportInfo('non_residential_plastic'),
    ...
    let ExportArray = [];
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_PLASTIC_NON_RESIDENTIAL, ACTION_MAPPING.REPORT_PLASTIC_NON_RESIDENTIAL.TABLE_COLUMN)) {
        viewColumnFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_PLASTIC_NON_RESIDENTIAL, ACTION_MAPPING.REPORT_PLASTIC_NON_RESIDENTIAL.TABLE_FILTER)) {
        filterFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_PLASTIC_NON_RESIDENTIAL_EXPORT, ACTION_MAPPING.REPORT_PLASTIC_NON_RESIDENTIAL_EXPORT.PDF)) {
        report.pdf = getPdfTableData({ columns, data: data });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_PLASTIC_NON_RESIDENTIAL_EXPORT, ACTION_MAPPING.REPORT_PLASTIC_NON_RESIDENTIAL_EXPORT.EXCEL)) {
        report.excel = getExcelTableData({ columns, data: data });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_PLASTIC_NON_RESIDENTIAL, ACTION_MAPPING.REPORT_PLASTIC_NON_RESIDENTIAL.TABLE_EXPORT)) {
    ...
    return (
        <div>
            ...
            <MUIDataTable
                title={I18n.t('non_residential_plastic')}
                data={data}
                columns={columns}
                options={options}
            />
...
export default reduxForm({
    form: 'NonResidentialPlastic'
})(NonResidentialPlastic);
