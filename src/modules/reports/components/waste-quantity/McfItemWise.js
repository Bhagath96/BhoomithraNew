...
function McfItemWise() {
    ...
    let report = {
        head: getReportInfo('mcf_item_wise'),
    ...
    let ExportArray = [];
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ITEM_WISE_MCF, ACTION_MAPPING.REPORT_ITEM_WISE_MCF.TABLE_COLUMN)) {
        viewColumnFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ITEM_WISE_MCF, ACTION_MAPPING.REPORT_ITEM_WISE_MCF.TABLE_FILTER)) {
        filterFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ITEM_WISE_MCF_EXPORT, ACTION_MAPPING.REPORT_ITEM_WISE_MCF_EXPORT.PDF)) {
        report.pdf = getPdfTableData({ columns, data: data });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ITEM_WISE_MCF_EXPORT, ACTION_MAPPING.REPORT_ITEM_WISE_MCF_EXPORT.EXCEL)) {
        report.excel = getExcelTableData({ columns, data: data });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ITEM_WISE_MCF, ACTION_MAPPING.REPORT_ITEM_WISE_MCF.TABLE_EXPORT)) {
