...
function RevenueReport() {
    ...
    let report = {
        head: getReportInfo('revenue_report'),
    ...
    let ExportArray = [];
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_REVENUE, ACTION_MAPPING.REPORT_REVENUE.TABLE_COLUMN)) {
        viewColumnFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_REVENUE, ACTION_MAPPING.REPORT_REVENUE.TABLE_FILTER)) {
        filterFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_REVENUE_EXPORT, ACTION_MAPPING.REPORT_REVENUE_EXPORT.PDF)) {
        report.pdf = getPdfTableData({ columns, data: data });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_REVENUE_EXPORT, ACTION_MAPPING.REPORT_REVENUE_EXPORT.EXCEL)) {
        report.excel = getExcelTableData({ columns, data: data });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_REVENUE, ACTION_MAPPING.REPORT_REVENUE.TABLE_EXPORT)) {
