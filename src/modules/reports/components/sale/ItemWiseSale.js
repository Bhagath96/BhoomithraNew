...
function ItemWiseSale() {
    ...
    let report = {
        head: getReportInfo('item_wise_sale'),
    ...
    let ExportArray = [];
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ITEM_WISE_SALE, ACTION_MAPPING.REPORT_ITEM_WISE_SALE.TABLE_COLUMN)) {
        viewColumnFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ITEM_WISE_SALE, ACTION_MAPPING.REPORT_ITEM_WISE_SALE.TABLE_FILTER)) {
        filterFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ITEM_WISE_SALE_EXPORT, ACTION_MAPPING.REPORT_ITEM_WISE_SALE_EXPORT.PDF)) {
        report.pdf = getPdfTableData({ columns, data: data });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ITEM_WISE_SALE_EXPORT, ACTION_MAPPING.REPORT_ITEM_WISE_SALE_EXPORT.EXCEL)) {
        report.excel = getExcelTableData({ columns, data: data });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ITEM_WISE_SALE, ACTION_MAPPING.REPORT_ITEM_WISE_SALE.TABLE_EXPORT)) {
