...
function ItemWiseStock() {
    ...
    let report = {
        head: getReportInfo('item_wise_stock'),
    ...
    let ExportArray = [];
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ITEM_WISE_STOCK, ACTION_MAPPING.REPORT_ITEM_WISE_STOCK.TABLE_COLUMN)) {
        viewColumnFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ITEM_WISE_STOCK, ACTION_MAPPING.REPORT_ITEM_WISE_STOCK.TABLE_FILTER)) {
        filterFlag = true;
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ITEM_WISE_STOCK_EXPORT, ACTION_MAPPING.REPORT_ITEM_WISE_STOCK_EXPORT.PDF)) {
        report.pdf = getPdfTableData({ columns, data: data });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ITEM_WISE_STOCK_EXPORT, ACTION_MAPPING.REPORT_ITEM_WISE_STOCK_EXPORT.EXCEL)) {
        report.excel = getExcelTableData({ columns, data: data });
    }
    if (hasAccessPermission(RESOURCE_MAPPING.REPORT_ITEM_WISE_STOCK, ACTION_MAPPING.REPORT_ITEM_WISE_STOCK.TABLE_EXPORT)) {
