...
const validate = (values) => {
    ...
    if (!_.has(values, 'startVal') || _.get(values, 'startVal', null) === null) {
        _.set(errors, 'startVal', I18n.t('required', { type: I18n.t('start_value') }));
    }
    if (!_.has(values, 'endVal') || _.get(values, 'endVal', null) === null) {
        _.set(errors, 'endVal', I18n.t('required', { type: I18n.t('end_value') }));
    }
    if (!_.has(values, 'pricePerUnit') || _.get(values, 'pricePerUnit', null) === null) {
        _.set(errors, 'pricePerUnit', I18n.t('required', { type: I18n.t('price_unit') }));
    }
    return errors;
};
...
function CreateServiceChargeSlab(props) {
    ...
    useEffect(() => {
        if (id) {
            dispatch(Actions.getServiceChargeSlabById(id));
        }
    }, [id]);
    ...
    const submit = (values) => {
        if (id) {
            dispatch(Actions.editServiceChargeSlab(id, values));

        } else {
            dispatch(Actions.sentServiceChargeSlab(values));
...
//     form: 'CreateServiceChargeSlabForm',
//     enableReinitialize: true,

//     // validate
// })(CreateServiceChargeSlab);
export default connect(state => ({
    initialValues: state[STATE_REDUCER_KEY].getServiceChargeSlabById?.data
}))(reduxForm({
    form: 'CreateServiceChargeSlabForm',
    enableReinitialize: true,
    validate
