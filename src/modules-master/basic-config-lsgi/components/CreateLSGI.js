...
function CreateLSGI(props) {
    ...
    const { fetchStateList, fetchDistrictList, fetchLSGItypeList, fetchDistrictPanchayathList, fetchBlockPanchayathList, fetchLSGIsById: { data: LSGIDataByID } } = useSelector(state => state[constant.STATE_REDUCER_KEY]);
    useEffect(() => {
        if (LSGIDataByID) {
    ...
    useEffect(() => {
        if (LSGItypeValue === 'Panchayath') {
            dispatch(Actions.fetchDistrictPanchayath(districtId));
            dispatch(Actions.fetchBlockPanchayath(districtId));
        }
    }, [LSGItypeValue, districtId]);
    useEffect(() => {
    ...
    const submit = (values) => {
        ...
        if (values.name && values.code && values.sort) {
            if (id) {
                ...
                object.lsgiTypeId = values.lsgiType?.id ? values.lsgiType?.id : null;
                object.districtPanchayathId = values.districtPanchayath?.id ? values.districtPanchayath?.id : null;
                object.blockPanchayathId = values.blockPanchayath?.id ? values.blockPanchayath?.id : null;
                if (values.labels?.length > 0) {
                    object.labels = values.labels;
                    dispatch(Actions.updateLSGIsData(id, object));
                } else {
                ...
                object.sort = values.sort;
                object.districtId = values.district?.id;
                object.lsgiTypeId = values.lsgiType?.id;
                object.districtPanchayathId = values.districtPanchayath?.id;
                object.blockPanchayathId = values.blockPanchayath?.id;
                if (values.labels?.length > 0) {
                    object.labels = values.labels;
                    dispatch(Actions.saveLSGIsData(object));
                } else {
                    SetLabelError('At least one label must be entered');
                }
            }
        } else {
            SetFormError('check your entered data');
        }
    };
                ...
                <Grid container>
                    ...
                    </Grid>
                    {
                        (LSGItypeValue === 'Panchayath') ?
                            <Grid container>
                                <Grid item xs={6} sm={6} md={6} className={classes.item} style={{ marginBottom: '20px', maxHeight: '100%', overflow: 'scrollbars' }}>
                                    <Field name='districtPanchayath' label={I18n.t('district_panchayath')} type='text' component={renderSelect} style={{ position: 'absolute' }}
                                        onChange={(value) => {
                                            change('blockPanchayath', null);
                                            value;
                                        }} >
                                        {
                                            fetchDistrictPanchayathList?.data.length > 0 ? _.get(fetchDistrictPanchayathList, 'data', []) : []
                                        }
                                    </Field>
                                </Grid>
                                <Grid item xs={6} sm={6} md={6} className={classes.item} style={{ marginBottom: '20px', maxHeight: '100%', overflow: 'scrollbars' }}>
