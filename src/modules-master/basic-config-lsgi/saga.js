...
import * as COMMON_API from '../../modules/common/api';
...
function* loadBlockPanchayath(action) {
    ...
    yield call(saga.handleAPIRequest, COMMON_API.loadBlockPanchayath, { types, districtId });
}
function* loadDistrictPanchayath(action) {
    const types = [ActionTypes.LOAD_DISTRICT_PANCHAYATH_REQUEST, ActionTypes.LOAD_DISTRICT_PANCHAYATH_SUCCESS, ActionTypes.LOAD_DISTRICT_PANCHAYATH_FAILURE];
    const districtId = action.payload.data;
    yield call(saga.handleAPIRequest, COMMON_API.loadDistrictPanchayath, { types, districtId });
}
...
export default function* dynamicFormsSaga() {
    yield all([
        ...
        takeLatest(ActionTypes.LOAD_DISTRICT_PANCHAYATH, loadDistrictPanchayath)
    ]);
}
