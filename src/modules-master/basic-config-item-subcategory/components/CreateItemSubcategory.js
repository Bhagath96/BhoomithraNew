...
function CreateItemSubcategory(props) {
    const { id } = useParams();
    const dispatch = useDispatch();
    const [labelError, SetLabelError] = useState('');
    const [formError, SetFormError] = useState('');
    const [negativeRateFlag, SetNegativeRateFlag] = useState(false);
    ...
    useEffect(() => {
        if (id) {
            dispatch(Actions.fetchItemSubcategoryById(id));
        }
    }, [id]);
    useEffect(() => {
    ...
    const checkBoxClick = () => {
        SetNegativeRateFlag(!negativeRateFlag);
    ...
    const submit = (values) => {
        ...
        let object = {
            name: '',
            negativeRate: values?.negativeRate ? true : false,
            sort: 0,
            labels: []
        };
        if (values.name && values.sort) {
            if (id) {
                object.name = values.name ? values.name : '';
                object.sort = values.sort ? values.sort : 0;
                if (values.labels?.length > 0) {
                    object.labels = values.labels;
                    dispatch(Actions.updatesItemSubcategory(id, object));
                } else {
                ...
                if (values.labels?.length > 0) {
                    object.labels = values.labels;
                    dispatch(Actions.saveItemSubcategory(object));
                } else {
                    SetLabelError(I18n.t('At_least_one_label_must_be_entered'));
                }
        ...
        <CardComponent>
            <Form onSubmit={handleSubmit(submit)} autoComplete='off'>
                <Grid container>
                    ...
                    <Grid item xs={12} sm={12} md={12}>
                        <Field component="input" name='negativeRate' type="checkbox" onClick={() => checkBoxClick()} /><span>{I18n.t('negative_rate')}</span>
                    </Grid>
...
export default
    connect(state => ({
        initialValues: state[STATE_REDUCER_KEY]?.fetchItemCategoryById?.data
    }))(reduxForm({
        form: 'createItem',
