...
function CreateServiceConfig(props) {
    ...
    useEffect(() => {
        if (id) {
            dispatch(Actions.getServiceConfigById(id));
        }
    }, [id]);
    ...
    const submit = (values) => {
        if (id) {
            dispatch(Actions.editServiceConfig(id, values));

        } else {
            dispatch(Actions.sentServiceConfig(values));
...
export default connect(state => ({
    initialValues: state[STATE_REDUCER_KEY].getServiceConfigById?.data
}))(reduxForm({
    form: 'CreateServiceConfigForm',
