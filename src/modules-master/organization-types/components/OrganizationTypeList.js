...
function OrganizationTypeList(props) {
    ...
    const handleClick = () => {
        ...
        history.push('/admin/index/organization-type/create');
    };

    //function for edit
    const editPressed = (rowData) => {
        let id = rowData[0];
        history.push(`${PATH.ORGANIZATION_TYPE}/${id}`);
    };
    //function for delete
    ...
    const showActionMenu = hasAccessPermission(RESOURCE_MAPPING.ORGANIZATION_TYPE, ACTION_MAPPING.ORGANIZATION_TYPE.EDIT_ORGANIZATION_TYPE) || hasAccessPermission(RESOURCE_MAPPING.USER, ACTION_MAPPING.USER.DELETE_USER) || false;
