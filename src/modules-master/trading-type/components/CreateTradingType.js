...
function CreateTradingType(props) {
    ...
    useEffect(() => {
        if (id) {
            dispatch(Actions.getTradingTypeById(id));
        }
    }, [id]);
    ...
    const submit = (values) => {
        if (id) {
            dispatch(Actions.editTradingType(id, values));

        } else {
            dispatch(Actions.sentTradingType(values));
...
//     form: 'CreateTradingTypeForm',
//     enableReinitialize: true,

//     // validate
// })(CreateTradingType);
export default connect(state => ({
    initialValues: state[STATE_REDUCER_KEY].getTradingTypeById.data
}))(reduxForm({
    form: 'CreateTradingTypeForm',
    enableReinitialize: true,
    validate
