...
function CreateShopType(props) {
    ...
    useEffect(() => {
        if (id) {
            dispatch(Actions.getShopTypeById(id));
        }
    }, [id]);
    ...
    const submit = (values) => {
        if (id) {
            dispatch(Actions.editShopType(id, values));

        } else {
            dispatch(Actions.sentShopType(values));
...
export default connect(state => ({
    initialValues: state[STATE_REDUCER_KEY].getShopTypeById?.data
}))(reduxForm({
    form: 'CreateShopTypeForm',
    enableReinitialize: true,
    validate
