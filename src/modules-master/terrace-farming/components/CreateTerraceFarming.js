...
function CreateTerraceFarming(props) {
    ...
    useEffect(() => {
        if (id) {
            dispatch(Actions.getTerraceFarmingById(id));
        }
    }, [id]);
    ...
    const submit = (values) => {
        if (id) {
            dispatch(Actions.editTerraceFarming(id, values));

        } else {
            dispatch(Actions.sentTerraceFarming(values));
...
//     form: 'CreateTerraceFarmingForm',
//     enableReinitialize: true,

//     // validate
// })(CreateTerraceFarming);
export default connect(state => ({
    initialValues: state[STATE_REDUCER_KEY].getTerraceFarmingById?.data
}))(reduxForm({
    form: 'CreateTerraceFarmingForm',
    enableReinitialize: true,
    validate
