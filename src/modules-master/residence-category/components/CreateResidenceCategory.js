...
function CreateResidenceCategory(props) {
    ...
    useEffect(() => {
        if (id) {
            dispatch(Actions.getResidenceCategoryById(id));
        }
    }, [id]);
    ...
    const submit = (values) => {
        if (id) {
            dispatch(Actions.editResidenceCategory(id, values));

        } else {
            dispatch(Actions.sentResidenceCategory(values));
...
export default connect(state => ({
    initialValues: state[STATE_REDUCER_KEY].getResidenceCategoryById.data
}))(reduxForm({
    form: 'CreateResidenceCategoryForm',
