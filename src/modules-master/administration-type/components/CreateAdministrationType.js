...
function CreateAdministrationType(props) {
    ...
    useEffect(() => {
        if (id) {
            dispatch(Actions.getAdministrationTypeById(id));
        }
    }, [id]);
    ...
    const submit = (values) => {
        if (id) {
            dispatch(Actions.editAdministrationType(id, values));

        } else {
            dispatch(Actions.sentAdministrationType(values));
...
export default connect(state => ({
    initialValues: state[STATE_REDUCER_KEY].getAdministrationTypeById?.data
}))(reduxForm({
    form: 'CreateAdministrationTypeForm',
