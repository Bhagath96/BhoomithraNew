...
const validate = (values) => {
    ...
    if (!_.has(values, 'residenceCategory') || _.get(values, 'residenceCategory', null) === null) {
...
function CreateBuildingType(props) {
    const dispatch = useDispatch();
    const { id } = useParams();
    const { handleSubmit, submitting, change } = props;
    useEffect(() => {
        dispatch(loadLanguagesForTitle());
        dispatch(Actions.loadResidentialCategories());
    }, []);
    useEffect(() => {
        if (id) {
            dispatch(Actions.getBuildingTypeById(id));
        }
    }, [id]);
    const titleDetails = useSelector(state => state.title);
    const buildingTypeDetails = useSelector(state => state[STATE_REDUCER_KEY]);
    const { getResidentialCategory } = buildingTypeDetails;

    const { languages } = titleDetails;
    // change('type', 1);
    const submit = (values) => {
        if (id) {
            dispatch(Actions.editBuildingType(id, { ...values, residenceCategoryId: values.residenceCategory.id }));

        } else {
            dispatch(Actions.sentBuildingType(values));
        ...
        <CardComponent >
            <Form onSubmit={handleSubmit(submit)}>
                <Grid container>
                    ...
                    <Grid item xs={6} className='fieldContainer'>
                        <Field name='residenceCategory' label={I18n.t('residence_category')} component={renderSelect}
                            onChange={(value) => {
                                change('residenceCategoryId', value.id);
                            }}

                        >
                            {
                                getResidentialCategory?.data.length > 0 ? _.get(getResidentialCategory, 'data', []) : []
                            }
                        </Field>
                    </Grid>
                </Grid>
...
//     form: 'CreateBuildingTypeForm',
//     enableReinitialize: true,

//     // validate
// })(CreateBuildingType);
export default connect(state => ({
    initialValues: state[STATE_REDUCER_KEY].getBuildingTypeById?.data ? state[STATE_REDUCER_KEY].getBuildingTypeById?.data : {}
}))(reduxForm({
    form: 'CreateBuildingTypeForm',
    enableReinitialize: true,
    validate
