...
import { FILE_UPLOAD_FORMAT } from '../../../modules/common/constants';
...
function CreateBundledServiceConfig(props) {
    const dispatch = useDispatch();
    const { id } = useParams();
    const { handleSubmit, submitting, change, dirty } = props;
    useEffect(() => {
        dispatch(loadLanguagesForTitle());
    }, []);
    useEffect(() => {
        if (id) {
            dispatch(Actions.getBundledServiceConfigById(id));
        }
    }, [id]);
    ...
    const submit = (values) => {
        if (id) {
            dispatch(Actions.editBundledServiceConfig(id, values));

        } else {
            dispatch(Actions.sentBundledServiceConfig(values));
    ...
    useEffect(() => {
        if (dirty) {
            dispatch(Action.setFormChange({ form: 'CreateBundledServiceConfigForm', isDirty: true }));
        }
    }, [dirty]);
    return (
        <CardComponent >
            <Form onSubmit={handleSubmit(submit)}>
                <Grid container>
                    ...
                    <Grid item xs={12} sm={12} md={12} className='fieldContainer' >
                        ...
                        <ImageUpload withoutMime={true} accept={`${FILE_UPLOAD_FORMAT.PNG},${FILE_UPLOAD_FORMAT.SVG},${FILE_UPLOAD_FORMAT.ICO}`} onImageUpload={(val) => {
                            change('icon', val);
                        }} />
                    </Grid>
                </Grid>
...
export default connect(state => ({
    initialValues: state[STATE_REDUCER_KEY].getBundledServiceConfigById?.data,
    dirty: isDirty('CreateBundledServiceConfigForm')
}))(reduxForm({
    form: 'CreateBundledServiceConfigForm',
    enableReinitialize: true,
    validate
})(CreateBundledServiceConfig));
