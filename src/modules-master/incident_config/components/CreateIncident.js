...
function CreateIncidentConfig(props) {
    ...
    useEffect(() => {
        if (id) {
            dispatch(Actions.getIncidentById(id));
        }
    }, [id]);
    ...
    const submit = (values) => {
        if (id) {
            dispatch(Actions.editIncident(id, values));

        } else {
            dispatch(Actions.saveIncident(values));
...
export default connect(state => ({
    initialValues: state[STATE_REDUCER_KEY].getIncidentById?.data
}))(reduxForm({
    form: 'CreateShopTypeForm',
