...
function CreateBlockPanchayath(props) {
    ...
    const { fetchStateList, fetchDistrictList } = useSelector(state => state[constant.STATE_REDUCER_KEY]);
        ...
        if (values.name && values.code && values.sort) {
            if (id) {
                ...
                if (values.labels?.length > 0) {
                    ...
                    dispatch(Actions.updateBlockPanchayathData(id, object));
                } else {
                ...
                if (values.labels?.length > 0) {
                    object.labels = values.labels;
                    dispatch(Actions.saveBlockPanchayathData(object));
                } else {
                    SetLabelError(I18n.t('At_least_one_label_must_be_entered'));
