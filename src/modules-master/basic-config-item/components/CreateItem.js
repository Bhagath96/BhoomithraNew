...
function CreateItem(props) {
    ...
    useEffect(() => {
        if (id) {
            dispatch(Actions.fetchItemById(id));
        }
    }, [id]);
    useEffect(() => {
        ...
        if (values.name && values.sort) {
            if (id) {
                ...
                if (values.labels?.length > 0) {
                    ...
                    dispatch(Actions.updatesItemData(id, object));
                } else {
                ...
                if (values.labels?.length > 0) {
                    object.labels = values.labels;
                    dispatch(Actions.saveItemData(object));
                } else {
                    SetLabelError(I18n.t('At_least_one_label_must_be_entered'));
                }
...
export default
    connect(state => ({
        initialValues: state[STATE_REDUCER_KEY].fetchItemById.data
    }))(reduxForm({
        form: 'createItem',
