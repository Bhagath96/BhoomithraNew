...
const renderTextField = ({ input: { value, name, ...inputProps }, min = 0, type = 'text', disabled, shrink = true, label, placeholder = '', adornment = {}, meta: { touched, error } }) => {
    const {
...
const reactMultiSelect = ({ input: { value, name, onBlur, ...inputProps }, limit = 0, option = [], label, meta: { touched, error }, multiple = true }) => {
    return (
        <FormControl style={styles.FormControl} fullWidth={true}>
            ...
            < ReactSelect
                ...
                isOptionDisabled={() => value.length === ((!multiple) ? 1 : limit === 0 ? option.length : limit)}
                {...inputProps}
            />
...
export { renderTextField, renderSelect, renderSimpleSelect, renderMultiSelect, renderTextAreaField, reactMultiSelect };
