            ...
            {formateBreadCrumpArray?.breadCrumpArray?.length > 0 ?
                <ul style={{ display: 'flex', listStyle: 'none' }}>
                    {formateBreadCrumpArray?.breadCrumpArray?.map((item, index) => {
                        let path = formateBreadCrumpArray?.breadCrumpArray?.length - 1 === index ? false : true;
                        return path ? <Fragment key={index}>
                            <Link className={classes.pointer} to={item?.path}>
                                <Typography>{item && item.label ? I18n.t(item.label) : ''}</Typography>
                            </Link>
